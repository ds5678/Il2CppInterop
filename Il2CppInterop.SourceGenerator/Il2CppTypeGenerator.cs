...
public sealed class Il2CppTypeGenerator : IIncrementalGenerator
{
    ...
    private static ClassModel? GetClassModel(GeneratorAttributeSyntaxContext ctx, CancellationToken ct)
    {
        if (ctx.TargetSymbol is not INamedTypeSymbol type)
            return null;
        if (ctx.TargetNode is not (ClassDeclarationSyntax or InterfaceDeclarationSyntax))
            return null;
        ...
        var finalizerMethods = type.GetMembers()
        ...
        foreach (var member in type.GetMembers().OfType<IPropertySymbol>())
        {
            ...
            if (managed && member.IsStatic) continue;
            ...
            fields.Add(new FieldModel(
                ...
                Index: index++,
                Accessibility: !explicitAccess ? null : member.DeclaredAccessibility,
        ...
        return new ClassModel(
            ...
            FinalizerMethodNames: finalizerMethods,
    ...
    private static void EmitPartialClass(IndentedTextWriter writer, ClassModel model)
    {
        ...
        writer.WriteLine($"{model.ClassAccessibility.GetAccessibilityKeyword()} partial {typeKindKeyword} {model.ClassName} : global::Il2CppInterop.Common.IIl2CppType<{model.ClassName}>");
        writer.WriteLine("{");
        writer.Indent++;
    ...
    {
        var accessPrefix = field.Accessibility is { } a ? a.GetAccessibilityKeyword() + " " : "";
    ...
    {
        var access = model.ClassAccessibility.GetAccessibilityKeyword();
...
sealed record ClassModel(
    ...
    ImmutableArray<string> FinalizerMethodNames,
